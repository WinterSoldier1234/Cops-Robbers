...
AddEventHandler('cnr:crimefree', function(enabled, resname)
  ...
  if enabled then
    if GetConvar("showdebugprintings", "false") == "true" then
      print("DEBUG - Crime Reporting: OFF")
    end
                    ...
                    if dist < 120.0 then
                      if HasEntityClearLosToEntity(ped, aimTarget, 17) then
                        if lastAim < GetGameTimer() then
                          ...
                          if GetConvar("showdebugprintings", "false") == "true" then
                            print("DEBUG - Aim crime reported too recently.")
              end
                        end
                      else
                        if GetConvar("showdebugprintings", "false") == "true" then
                          print("DEBUG - No line of sight")
              end
                      end
                    else
                      if GetConvar("showdebugprintings", "false") == "true" then
                        print("DEBUG - Too far away.")
              end
                    end
                  else
                    if GetConvar("showdebugprintings", "false") == "true" then
                      print("DEBUG - Entity not a ped")
              end
                  end
                else
                  if GetConvar("showdebugprintings", "false") == "true" then
                    print("DEBUG - Nonexistant entity.")
              end
                end
              else
                if GetConvar("showdebugprintings", "false") == "true" then
                  print("DEBUG - Ignoring freeaim; In a crime free zone")
              end
              end
            else
              if GetConvar("showdebugprintings", "false") == "true" then
                print("DEBUG - Not a crime to aim with this weapon ("..tostring(GetSelectedPedWeapon(PlayerPedId()))..").")
              end
            end
          end
        else
          if GetConvar("showdebugprintings", "false") == "true" then
            print("DEBUG - Ignoring freeaim; Player is a cop.")
              end
        end
