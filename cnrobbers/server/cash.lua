...
function SetPlayerWallet(ply, value, exactValue)
  ...
  if exactValue then 
    MySQL.execute.await("UPDATE characters SET cash = @v WHERE idUnique = @u",
      {['v'] = value, ['u'] = UniqueId(ply)}
    )
  else
    MySQL.execute.await("UPDATE characters SET cash = cash + @v WHERE idUnique = @u",
      {['v'] = value, ['u'] = UniqueId(ply)}
    )
  end
  SyncPlayerMoney(ply)
...
function SetPlayerBalance(ply, value, exactValue)
  if not ply then return 0 end
  if exactValue then 
    MySQL.execute.await("UPDATE characters SET bank = @v WHERE idUnique = @u",
      {['v'] = value, ['u'] = UniqueId(ply)}
    )
  else
    MySQL.execute.await("UPDATE characters SET bank = bank + @v WHERE idUnique = @u",
      {['v'] = value, ['u'] = UniqueId(ply)}
    )
  end
  SyncPlayerMoney(ply)
...
function GetPlayerCash(ply)
  ...
  return MySQL.execute.await("SELECT cash FROM characters WHERE idUnique = @u",{['u']=uid})
end
...
function GetPlayerBank(ply)
  ...
  local uid = UniqueId(ply)
  if uid < 1 then return 0 end
  return MySQL.execute.await("SELECT bank FROM characters WHERE idUnique = @u",{['u']=uid})
end
...
function BankTransaction(ply, value, paySource)
  ...
  local uid = UniqueId(ply)
	local bank = MySQL.execute.await(
...
function CashTransaction(ply, value, payTitle)
  ...
  if type(value) ~= "number" then value = tonumber(value) end
	...
	local uid  = UniqueId(ply)
	local cash = MySQL.execute.await(
